    ...
    try {
      const query = `
        SELECT 
          ...
          i.stock_id,
          i.stock_name,
          COALESCE(SUM(id.remaining_quantity), 0) AS quantity,
      ...
      db.query(insertInventory, [stock_id, stock_name, category_id, unit, threshold], (err, result) => {
        ...
        const insertDetails = `
          INSERT INTO inventory_details (
            inventory_id, delivery_date, quantity, unit, expiration_date
          ) VALUES (?, ?, ?, ?, ?)
        `;

        db.query(insertDetails, [inventoryId, delivery_date, quantity, unit, expiration_date], (err) => {
          if (err) {
            return db.rollback(() => res.status(500).json({ error: err.message }));
          }
