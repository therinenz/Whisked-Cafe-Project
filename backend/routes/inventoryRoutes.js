...
const validateInventoryInput = [
  ...
  body('unit').isIn(['kg', 'liters', 'units', 'pieces']), // adjust units as needed
];
...
WHERE i.archive = 0
GROUP BY i.id, i.stock_id, i.stock_name;
      ...
      if (err) {
        ...
        res.json(
          results.map((item) => ({
            ...
            quantity: `${item.remaining_quantity} / ${item.initial_quantity}`, // Format for frontend
...
router.post('/', validateInventoryInput, (req, res) => {
  ...
  const insertInventory = `
    INSERT INTO inventory (
      stock_id, stock_name, category_id, unit, threshold
    ) VALUES (?, ?, ?, ?, ?)
...
// Add this new route for deducting stock
...
// Add this new route for POS-triggered deductions
...
// Add a new endpoint to clean up empty stocks
...
// GET specific inventory by ID (place this BEFORE the catch-all route)
router.get('/:id', async (req, res) => {
  console.log('Accessing inventory ID:', req.params.id);
  try {
    const inventoryId = req.params.id;
    const query = `
      SELECT 
